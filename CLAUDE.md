...
## 端口配置
**重要**：本应用运行在 8080 端口。所有服务应保持一致的配置：
- 服务器端口：8080（在 config.yaml 中配置）
- PostgreSQL：5432（Docker 容器映射到主机）
- Redis：6379（Docker 容器映射到主机）
...
通过 `make compose` 可用的服务：
- **PostgreSQL 15**：主数据库
- **Redis 7**：缓存层
- **pgAdmin**：数据库管理界面（localhost:5050）
- **Redis Commander**：Redis 管理界面（localhost:8081）
- **Nginx**：反向代理
- **Prometheus/Grafana**：监控栈（可选）
...
### 服务依赖
- 运行 `make dev` 前务必先用 `make compose` 启动服务
- 必要时用单独容器命令检查服务健康状态
